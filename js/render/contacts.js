...
function renderMap(data, lang) {
  ...
  const lat = Number(data.mapLat) || 48.5248434;
  const lng = Number(data.mapLng) || 35.0227965;
  const zoom = Number(data.mapZoom) || 17;
  const hl = lang === "en" ? "en" : "uk";
  const label = pickLocalized(data, lang, "mapLabel");
  const address = pickLocalized(data, lang, "address");
  const query = address
    ? encodeURIComponent(`${address}, ${lang === "en" ? "Ukraine" : "Україна"}`)
    : `${lat},${lng}`;
  ...
  iframe.src = `https://www.google.com/maps?q=${query}&hl=${hl}&z=${zoom}&output=embed`;
